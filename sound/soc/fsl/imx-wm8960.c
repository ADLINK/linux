		...
		struct snd_soc_jack_gpio *gpio)
{
	int ret;
	//
	//int snd_soc_card_jack_new(struct snd_soc_card *card, const char *id, int type, struct snd_soc_jack *jack)
	//
	ret = snd_soc_card_jack_new(card, pin->pin, pin->mask, jack);
	if (ret) {
		return ret;
	}
...
static int imx_wm8960_probe(struct platform_device *pdev)
{
	...
	data->imx_hp_jack_gpio.gpio = of_get_named_gpio(pdev->dev.of_node,
							      "hp-det-gpios", 0);

	data->imx_mic_jack_gpio.gpio = of_get_named_gpio(pdev->dev.of_node,
							       "mic-det-gpios", 0);
...
static const struct of_device_id imx_wm8960_dt_ids[] = {
	{ .compatible = "fsl,imx-audio-wm8960-ad", },
